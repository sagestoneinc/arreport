...
describe('formatDailyBatchReruns', () => {
  it('generates correct output for Daily Batch Reruns with sample data', () => {
    ...
    // Check content
    expect(output).toContain('I re-ran 100 US/CA declines from yesterday to PAY-REV and got 25 sales (25% approval).');
    expect(output).toContain('Visa: 30% (15 approvals, 50 txns)');
    expect(output).toContain('MC: 20% (10 approvals, 50 txns)');
    expect(output).toContain('Common Declines: Do Not Honor (40%), Insufficient Funds (30%), Invalid Card (20%)');
    expect(output).toContain('I re-ran 50 declines (all other geos) to NS and got 10 sales (20% approval).');
  });
  ...
  it('generates correct output with template placeholder', () => {
    const state: AppState = {
      ...
      visaMids: [],
      mcMids: [],
      notes: 'I re-ran __ US/CA declines from yesterday to PAY-REV and got __ sales (__ approval).\n\nVisa: __ (__ approvals, __ txns)\nMC: __ (__ approvals, __ txns)\nCommon Declines: __ (), __ (), __ ()',
    };
    ...
    expect(output).toContain('Daily Batch Re-runs Summary: 01/24/2026');
    expect(output).toContain('I re-ran __ US/CA declines from yesterday to PAY-REV and got __ sales (__ approval).');
