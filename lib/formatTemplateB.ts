...
/**
 * Format the Template B message (Threshold Performing/Low format)
 * Output MID line format: "- {MID}: {AR}% ({sales} / {declines})"
 */
export function formatTemplateB(state: AppState): string {
  const lines: string[] = [];
